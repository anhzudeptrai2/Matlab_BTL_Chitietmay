...
% Thông số từ bảng
P1 =0.9; % kW - Công suất trên trục chủ động
n1 = 1450; % vòng/phút - Số vong quay trên trục chủ động
T1 = 5927.6; % N.mm - Moment xoắn của trục chủ động
u = 2.5; % Tỉ số truyền của bộ truyền

% Thông số thiết kế
ca = 3; % ca làm việc
beta_goc = 20; % độ - Góc nghiêng bộ truyền so với phương nằm ngang
pi = 3.14; % Hằng số Pi
...
% 1.1: Chọn loại đai
if(P1 < 2 && n1 == 1450)
    fprintf('1.1: Chọn loại đai: Đai thang thường loại 0\n');
    fprintf('Loại đai thang thường loại O phù hợp với công suất P1 < 2 kW và n1 < 1500 vòng/phút\n');
else
...
%Chọn chiều dài đai theo dãy tiêu chuẩn (chọn giá trị lớn hơn hoặc bằng và gần nhất)
...
if ~isempty(idx)
    ...
    fprintf('Chọn chiều dài đai tiêu chuẩn lớn hơn hoặc bằng và gần nhất: L = %d mm\n', L_chon);
else
